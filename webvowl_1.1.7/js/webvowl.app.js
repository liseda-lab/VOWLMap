webvowl.app =
    ...
    ({
        ...
        /***/
            ...
            /* WEBPACK VAR INJECTION */
            (function(d3) { /* Taken from here: http://stackoverflow.com/a/17907562 */
            ...
            (function(d3) {
                ...
                module.exports = function() {
                    ...
                    var app = {},
                        ...
                        alignmentPane = __webpack_require__(345)(graph),
                        srcSearchMenu = __webpack_require__(346)(graph),
                        tgtSearchMenu = __webpack_require__(347)(graph),
                        ...
                        mappingFilter = webvowl.modules.mappingFilter();
                    ...
                    function addFileDropEvents(selector) {
                        ...
                        node.node().ondragover = function(e) {
                            ...
                            if ((cx > 0.25 * w && cx < 0.75 * w) && (cy > 0.25 * h && cy < 0.75 * h)) {
                                ...
                                d3.select("#drag_msg_text").node().innerHTML = "Drag alignment file here.";
                    ...
                    app.initialize = function() {
                        ...
                        options.filterModules().push(mappingFilter);

                        d3.select(window).on("resize", adjustSize);

                        exportMenu.setup();
                        gravityMenu.setup();
                        filterMenu.setup(datatypeFilter, objectPropertyFilter, subclassFilter, disjointFilter, setOperatorFilter, nodeDegreeFilter, mappingFilter);
                        ...
                        if (agentVersion > 0 && agentVersion <= 11) {
                            ...
                            options.srcSearchMenu(srcSearchMenu);
                            options.tgtSearchMenu(tgtSearchMenu);
                            options.ontologyMenu(ontologyMenu);
                            options.navigationMenu(navigationMenu);
                            options.sidebar(sidebar);
                            options.leftSidebar(leftSidebar);
                            options.alignmentPane(alignmentPane);
                            ...
                            options.disjointFilter(disjointFilter);
                            ...
                            options.mappingFilter(mappingFilter);
                    ...
                    function loadOntologyFromText(jsonText, filename, alternativeFilename) {
                        ...
                        if (classCount === 0 && graph.editorMode() === false && loadEmptyOntologyForEditing === false) {
                            ...
                            // var flagOfCheckBox2 = d3.select("#mapModeModuleCheckbox").node().checked;
                            graph.editorMode(false); // update gui
                            graph.mapMode(true);
            ...
            (function(d3) {
                ...
                module.exports = function(graph) {
                    ...
                    exportMenu.setup = function() {
                        ...
                        exportJsonAlignButton = d3.select("#exportJsonAlignment")
                            .on("click", exportJsonAlignment);

                        exportRDFAlignButton = d3.select("#exportRDFAlignment")
                            .on("click", exportRDFAlignment);
                    ...
                    exportMenu.setFilename = function(filename) {
                        exportFilename = filename.split(".")[0] || "export";
                        var today = new Date();
                        ...
                        today = mm + '/' + dd + '/' + yyyy;
                        if (!exportFilename.includes(today)) {
                    ...
                    exportMenu.exportAsUrl = function() {
                        ...
                        currObj.filter_mapping = String(graph.options().filterMenu().getCheckBoxValue("mappingFilterCheckbox"));
                    ...
                    function exportJsonAlignment() {
                        ...
                        var allJSONexportObj = exportMenu.createJSON_exportObject();
                        ...
                        exportObj._comment = allJSONexportObj._comment;
                        exportObj.header = allJSONexportObj.header;
                        exportObj.namespace = allJSONexportObj.namespace;
                        exportObj.metrics = allJSONexportObj.metrics;

                        for (i = 0; i < allJSONexportObj.propertyAttribute.length; i++) {
                            if (allJSONexportObj.propertyAttribute[i].label) {
                                if (allJSONexportObj.propertyAttribute[i].label.undefined === "mapping" || allJSONexportObj.propertyAttribute[i].label == "mapping") {
                                    propertyAttributeObjects.push(allJSONexportObj.propertyAttribute[i])
                                }
                            }
                        }

                        for (i = 0; i < allJSONexportObj.property.length; i++) {
                            for (j = 0; j < propertyAttributeObjects.length; j++) {
                                if (allJSONexportObj.property[i].id === propertyAttributeObjects[j].id) {
                                    if (!propertyObjects.includes(allJSONexportObj.property[i])) {
                                        propertyObjects.push(allJSONexportObj.property[i])
                                    }
                                }
                            }
                        }

                        for (i = 0; i < allJSONexportObj.class.length; i++) {
                            for (j = 0; j < propertyAttributeObjects.length; j++) {
                                if (allJSONexportObj.class[i].id === propertyAttributeObjects[j].range || allJSONexportObj.class[i].id === propertyAttributeObjects[j].domain) {
                                    if (!classObjects.includes(allJSONexportObj.class[i])) {
                                        classObjects.push(allJSONexportObj.class[i])
                                    }
                                }
                            }
                        }

                        for (i = 0; i < allJSONexportObj.classAttribute.length; i++) {
                            for (j = 0; j < classObjects.length; j++) {
                                if (allJSONexportObj.classAttribute[i].id === classObjects[j].id) {
                                    if (!classAttributeObjects.includes(allJSONexportObj.classAttribute[i])) {
                                        classAttributeObjects.push(allJSONexportObj.classAttribute[i])
                                    }
                                }
                            }
                        }
                        exportObj.class = classObjects;
                        exportObj.classAttribute = classAttributeObjects;
                        exportObj.property = propertyObjects;
                        exportObj.propertyAttribute = propertyAttributeObjects;
                        ...
                        if (!jsonExportFileName.endsWith(".json"))
                            jsonExportFileName += ".json";
                        exportJsonAlignButton.attr("href", dataURI)
                            .attr("download", jsonExportFileName);
                    }

                    function exportRDFAlignment() {
                        graph.options().navigationMenu().hideAllMenus();
                        /**  check if there is data **/
                        if (!exportableJsonText) {
                            alert("No graph data available.");
                        ...
                        var allJSONexportObj = exportMenu.createJSON_exportObject();
                        var onto1 = allJSONexportObj.header.iri.split("_")[0].toLowerCase() + ".owl"
                        var onto2 = "http://" + allJSONexportObj.header.iri.split("_")[1].toLowerCase()
                        var onto1noOWL = onto1.split(".owl")[0]
                        var onto2noOWL = onto2.split(".owl")[0]
                        var exportText = "";
                        exportText += "<?xml version='1.0' encoding='utf-8'?> \n<rdf:RDF xmlns='http://knowledgeweb.semanticweb.org/heterogeneity/alignment'\n\txmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#'\n\txmlns:xsd='http://www.w3.org/2001/XMLSchema#'>\n"
                        exportText += "\n<Alignment>\n"
                        exportText += "\t<xml>yes</xml>\n"
                        exportText += "\t<level>0</level>\n"
                        exportText += "\t<type>??</type>\n"
                        exportText += "\t<onto1>" + onto1 + "</onto1>\n",
                            exportText += "\t<onto2>" + onto2 + "</onto2>\n"
                        exportText += "\t<uri1>" + onto1 + "</uri1>\n"
                        exportText += "\t<uri2>" + onto2 + "</uri2>\n"
                        ...
                        for (i = 0; i < allJSONexportObj.propertyAttribute.length; i++) {
                            if (allJSONexportObj.propertyAttribute[i].label) {
                                inverseIds.push(allJSONexportObj.propertyAttribute[i].inverse)
                                if (allJSONexportObj.propertyAttribute[i].label.undefined === "mapping" || allJSONexportObj.propertyAttribute[i].label == "mapping") {
                                    if (!inverseIds.includes(allJSONexportObj.propertyAttribute[i].id)) {
                                        propertyAttributeObjects.push(allJSONexportObj.propertyAttribute[i])
                                    }
                                }
                            }
                        ...
                        for (i = 0; i < allJSONexportObj.classAttribute.length; i++) {
                            for (j = 0; j < propertyAttributeObjects.length; j++) {
                                if (allJSONexportObj.classAttribute[i].id === propertyAttributeObjects[j].range || allJSONexportObj.classAttribute[i].id === propertyAttributeObjects[j].domain) {
                                    if (!classAttributeObjects.includes(allJSONexportObj.classAttribute[i])) {
                                        classAttributeObjects.push(allJSONexportObj.classAttribute[i])
                                    }
                                }
                        ...
                        var mappings = []
                        for (i = 0; i < propertyAttributeObjects.length; i++) {
                            for (j = 0; j < classAttributeObjects.length; j++) {
                                if (classAttributeObjects[j].iri.split("#")[0].toLowerCase().search(".owl") === -1) {
                                    if (propertyAttributeObjects[i].range === classAttributeObjects[j].id && classAttributeObjects[j].iri.split("#")[0].toLowerCase().split(".owl")[0] === onto1noOWL) {
                                        var range = classAttributeObjects[j].iri
                                    }
                                    if (propertyAttributeObjects[i].domain === classAttributeObjects[j].id && classAttributeObjects[j].iri.split("#")[0].toLowerCase().split(".owl")[0] === onto2noOWL) {
                                        var domain = classAttributeObjects[j].iri
                                    }
                                } else {
                                    if (propertyAttributeObjects[i].range === classAttributeObjects[j].id && classAttributeObjects[j].iri.split("#")[0].toLowerCase() === onto1) {
                                        var range = classAttributeObjects[j].iri
                                    }
                                    if (propertyAttributeObjects[i].domain === classAttributeObjects[j].id && classAttributeObjects[j].iri.split("#")[0].toLowerCase() === onto2) {
                                        var domain = classAttributeObjects[j].iri
                                    }
                                }
                            ...
                            mapsObj = { range: range, domain: domain, score: propertyAttributeObjects[i].annotations.score[0].value, status: propertyAttributeObjects[i].annotations.status[0].value }
                            if (!mappings.includes(mapsObj)) {
                                mappings.push(mapsObj)
                            }
                        }
                        ...
                        for (i = 0; i < mappings.length; i++) {
                            exportText += "\t<map>\n"
                            exportText += "\t\t<Cell>"
                            exportText += '\n\t\t\t<entity1 rdf:resource="' + mappings[i].range + '"/>'
                            exportText += '\n\t\t\t<entity2 rdf:resource="' + mappings[i].domain + '"/>'
                            exportText += '\n\t\t\t<measure rdf:datatype="http://www.w3.org/2001/XMLSchema#float">' + mappings[i].score + "</measure>"
                            exportText += "\n\t\t\t<relation>=</relation>"
                            exportText += "\n\t\t\t<revision>"
                            exportText += "\n\t\t\t\t<status>" + mappings[i].status + "</status>"
                            exportText += "\n\t\t\t</revision>"
                            exportText += "\n\t\t</Cell>"
                            exportText += "\n\t</map>\n\n"
                        }
                        ...
                        // make a string again;
                        exportText += "</Alignment>"
                        exportText += "\n</rdf:RDF>"
                            // write the data
                        var dataURI = "data:text;charset=utf-8," + encodeURIComponent(exportText);
                        var jsonExportFileName = exportFilename.split(".")[0];
                        if (!jsonExportFileName.endsWith(".rdf"))
                            jsonExportFileName += ".rdf";
                        exportRDFAlignButton.attr("href", dataURI)
                            .attr("download", jsonExportFileName);
                    }
        ...
        /***/
            ...
            (function(d3) {
                ...
                module.exports = function(graph) {
                     ...
                     * @param mappingFilter
                     */
                    filterMenu.setup = function(datatypeFilter, objectPropertyFilter, subclassFilter, disjointFilter, setOperatorFilter, nodeDegreeFilter, mappingFilter) {
            ...
            (function(d3) {
                ...
                module.exports = function(graph) {
                    ...
                    modeMenu.setup = function(pickAndPin, nodeScaling, compactNotation, colorExternals) {
                        ...
                        addCheckBox("mapMode", "Alignment", "#mapMode", graph.mapMode);
                    ...
                    function addCheckBox(identifier, modeName, selector, onChangeFunc) {
                        ...
                        moduleCheckbox.on("click", function(d) {
                            var isEnabled = moduleCheckbox.property("checked");
                            onChangeFunc(isEnabled);
                            // if (isEnabled === true & identifier === "editorMode")
                ...
                module.exports = function(graph) {
                    ...
                    function append_bulletPoint(msg) {
                        if (msg.length === 0) {
                            ...
                            bp_container.node().innerHTML = "<br><p>VOWLMAP runs in a local instance.</p> <p>CORS prevents to automatically load files on host system.</p><p>You can load preprocessed alignments using the upload feature in the ontology menu or by dragging the files and dropping them on the canvas.</p> <p><i>Hint: </i>Note that the conversion of ontologies and alignments into the VOWL-JSON format is not part of WebVOWL but requires an additional converter such as OWL2VOWL.</p> <p>Loading some mapping graphs may take some seconds.</p>"
                            loadingModule.scrollDownDetails();
                ...
                module.exports = function(graph) {
                    ...
                    function updateSearchDictionary() {
                        ...
                        for (i = 0; i < labelDictionary.length; i++) {
                            if (labelDictionary[i].labelForCurrentLanguage() === "mapping") {
                                var lEntry = labelDictionary[i].range().labelForCurrentLanguage() + "=" + labelDictionary[i].domain().labelForCurrentLanguage();
                    ...
                    function createDropDownElements() {
                        ...
                        for (i = 0; i < numEntries; i++) {
                            ...
                            //filter of elements
                            if (d3.select("#containerForAlignment").attr("class").includes("hidden")) {
                                if (eLen === 1 || allSame === true) {
                                    if (nodeMap[entries[0]] === undefined) {
                                        if (!newResults[i].includes("=")) {
                    ...
                    function handleClick(elementId) {
                        ...
                        return function() {
                            ...
                            if (correspondingStr.includes("=")) {
                                ...
                                graph.executeColorExternalsModule();
                                graph.executeCompactNotationModule();
                                graph.options().compactNotationModule().enabled(true);
                                graph.lazyRefresh();
                                alignmentPane.NodeDistanceDropdown(correspondingIds[0]);
                                d3.select("#degreeOption1").classed("selected", true);
                                graph.subgraphGenerator(correspondingIds[0], 1);
                                console.log(correspondingIds[0])
                ...
                module.exports = function(graph) {
                    ...
                    loadingModule.from_JSON_URL = function(fileName) {
                        ...
                        if (ontologyMenu.cachedOntology(filename)) {
                            ...
                            ontologyMenu.append_message("Retrieving alignment from JSON URL " + filename);
                    ...
                    loadingModule.from_IRI_URL = function(fileName) {
                        ...
                        if (ontologyMenu.cachedOntology(filename)) {
                            ...
                            ontologyMenu.append_bulletPoint("Retrieving alignment from IRI: " + filename);
                    ...
                    loadingModule.fromFileDrop = function(fileName, file) {
                        ...
                        ontologyMenu.append_bulletPoint("Retrieving alignment from dropped file: " + fileName);
                        var ontologyContent = "";
                    ...
                    loadingModule.from_FileUpload = function(fileName) {
                        ...
                        if (ontologyMenu.cachedOntology(filename)) {
                            ...
                            ontologyMenu.append_bulletPoint("Retrieving alignment from file: " + filename);
                            // get the file
                    ...
                    function fallbackConversion(parameter) {
                        ...
                        xhr.onreadystatechange = function() {
                            if (xhr.readyState === 4 && xhr.status === 0) {
                                ...
                                loadingModule.setErrorMode();
                                ontologyMenu.append_bulletPoint("Failed to load alignment");
                                ontologyMenu.append_message_toLastBulletPoint("<br><span style='color:red'>Could not connect to OWL2VOWL service </span>");
                                loadingModule.showErrorDetailsMessage();
                            }
                        };
                    ...
                    loadingModule.from_presetOntology = function(selectedOntology) {
                        if (selectedOntology !== DEFAULT_JSON_NAME) {
                            ontologyMenu.append_bulletPoint("Retrieving alignment: " + selectedOntology);
                        } else {
                            ...
                            d3.xhr(fileToRead, "application/json", function(error, request) {
                                ...
                                if (loadingSuccessful) {
                                    ...
                                    if (loadingNewOntologyForEditor) {
                                        ...
                                        } else if (ontology === DEFAULT_JSON_NAME) {
                                            if (window.location.toString().startsWith("file:/")) {
                                                d3.select("#layoutLoadingProgressBarContainer").classed("hidden", true);
                ...
                module.exports = function(graph) {
                    ...
                    warningModule.responseWarning = function(header, reason, action, callback, parameterArray, forcedWarning) {
                        ...
                        gotItButton.on("click", function() {
                            ...
                            callback(parameterArray[1]);
                            callback(parameterArray[0]);
                            callback(parameterArray[1]);
                            callback(parameterArray[0]);
                            graph.options().alignmentPane().setup();
        ...
        /***/
            ...
            (function(d3) {
                ...
                module.exports = function(graph) {
                    ...
                    var results = [];
                    var resultID = [];
                    var src_search = d3.select("#source_search"); // << dropdown container;

                    String.prototype.beginsWith = function(string) {
                        return (this.indexOf(string) === 0);
                    };

                    srcSearchMenu.requestDictionaryUpdate = function() {
                        dictionaryUpdateRequired = true;
                        // clear possible pre searched entries
                        var htmlCollection = src_search.node().children;
                        var numEntries = htmlCollection.length;
                    ...
                    srcSearchMenu.setup = function() {
                        // clear dictionary;
                        dictionary = [];
                        searchLineEdit = d3.select("#sname");
                        searchLineEdit.on("input", userInput);
                    ...
                    function hoverSearchEntryView() {
                        updateSelectionStatusFlags();
                        srcSearchMenu.showSearchEntries();
                    }

                    srcSearchMenu.hideSearchEntries = function() {
                        src_search.style("display", "none");
                        viewStatusOfSearchEntries = false;
                    };

                    srcSearchMenu.showSearchEntries = function() {
                        src_search.style("display", "block");
                        viewStatusOfSearchEntries = true;
                    };
                    ...
                    function createDropDownElements() {
                        ...
                        for (i = 0; i < numEntries; i++) {
                            ...
                            searchEntryNode.node().innerHTML = croppedText;
                            src_search.node().appendChild(testEntry);
                        }
                    }
                    ...
                    function handleClick(elementId) {
                        ...
                        return function() {
                            ...
                            srcSearchMenu.hideSearchEntries();
                            d3.select("#sname").attr("value", correspondingStr)
                            src_search.style("display", "none")
                        };
                    }

                    srcSearchMenu.clearText = function() {
                        searchLineEdit.node().value = "";
                        var htmlCollection = src_search.node().children;
                        var numEntries = htmlCollection.length;
                        for (var i = 0; i < numEntries; i++) {
