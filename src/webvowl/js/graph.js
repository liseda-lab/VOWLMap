...
module.exports = function (graphContainerSelector) {
  ...
  var shadowClone = require("./shadowClone")(graph);(graph);
  ...
  graph.graphContainer = function () {
    return graphContainer;
  };

  graph.classNodes = function () {
    return classNodes;
  };


  graph.properties = function () {
    return properties;
  };

  graph.nodeContainer = function () {
    return nodeContainer;
  };

  graph.linkContainer = function () {
    return linkContainer;
  };

  graph.labelContainer = function () {
    return labelContainer;
  };

  graph.getNewMapping = function () {
    return newMapping;
  };

  graph.getCurrentId = function () {
    return currentId;
  };
  graph.getCurrentDegree = function () {
      ...
      .on("dragend", function (d) {
        ...
        if (d.type && d.type() === "Class_dragger") {
          ...
          if (targetNode) {
            ...
            if (mappingMode === true) {
              createNewMappingViaEditor(d.parentNode(), targetNode, draggerEndPos)
              graph.fastUpdate()
              graph.options().alignmentPane().setup();
              if (currentMapping !== undefined) {
                graph.subgraphGenerator(currentId, currentDegree);
              }
          ...
          if (targetRangeNode === null) {
            ...
            if (d.parent.domain().attributes().includes("external") && targetRangeNode.attributes().includes("external")) {
              d.reDrawEverthing()
              shadowClone.hideParentProperty(false);
            } else if (!d.parent.domain().attributes().includes("external") && !targetRangeNode.attributes().includes("external")) {
              d.reDrawEverthing()
              shadowClone.hideParentProperty(false);
            } else {

              d.updateRange(targetRangeNode);
              d.parent.inverse().domain(targetRangeNode);
              shadowClone.hideParentProperty(false);
              if (mappingMode === true) {
                graph.fastUpdate();
                graph.options().alignmentPane().setup();
                graph.options().sidebar().updateSelectionInformation(d.parent);
                if (currentMapping !== undefined) {
                  graph.subgraphGenerator(currentId, currentDegree);
                }
              } else {
          ...
          if (targetDomainNode === null) {
            ...
            if (d.parent.range().attributes().includes("external") && targetDomainNode.attributes().includes("external")) {
              d.reDrawEverthing()
              shadowClone.hideParentProperty(false);
            } else if (!d.parent.range().attributes().includes("external") && !targetDomainNode.attributes().includes("external")) {
              d.reDrawEverthing()
              shadowClone.hideParentProperty(false);
            } else {
              d.updateDomain(targetDomainNode);
              d.parent.inverse().range(targetDomainNode);
              shadowClone.hideParentProperty(false);
              if (mappingMode === true) {
                graph.fastUpdate();
                graph.options().alignmentPane().setup();
  ...
  function addClickEvents() {
    ...
    nodeElements.on("dblclick", function (clickedNode) {
      ...
      d3.event.stopPropagation();
      if (editMode === true && mappingMode === false) {
        clickedNode.raiseDoubleClickEdit(defaultIriValue(clickedNode));
      }
    });
  ...
  function loadGraphData(init) {
    ...
    if (graphContainer && validOntology === true) {
      ...
      force.stop();
      graph.options().ontologyMenu().append_bulletPoint("Failed to load alignment");
      loadingModule.setErrorMode();
    }
    // update prefixList(
  ...
  ////mappings subgraphs
  graph.subgraphGenerator = function (id, degree) {
    var mapping;
    currentId = id;
    currentDegree = degree;

    var preprocessedData = _.clone(unfilteredData);
    for (i in preprocessedData.properties) {
      if (preprocessedData.properties[i].id() == id) {
        mapping = preprocessedData.properties[i]
      }
    }
    currentMapping = mapping

    function degreeSetter(n) {
      if (n == 0) {
        preprocessedData.properties = preprocessedData.properties.filter(function (x) {

          return x.id() == id;
        });
        return preprocessedData.properties;

      } else {
        preprocessedData.properties = preprocessedData.properties.filter(function (x) {
          for (var i in degreeSetter(n - 1)) {
            if (degreeSetter(n - 1)[i].id() === x.id()) {
              return x

            } else if (degreeSetter(n - 1)[i].range().id() === x.domain().id() || degreeSetter(n - 1)[i].range().id() === x.range().id() ||
              degreeSetter(n - 1)[i].domain().id() === x.domain().id() || degreeSetter(n - 1)[i].domain().id() === x.range().id()) {
              return x;
            }
          };
        });
    ...
    if (degree === 3) {
      preprocessedData.properties = degreeSetter(2)
      unfilteredData.properties.forEach(function (x) {
        for (var i in preprocessedData.properties) {
          if (preprocessedData.properties[i].range().id() === x.domain().id() || preprocessedData.properties[i].range().id() === x.range().id() ||
            preprocessedData.properties[i].domain().id() === x.domain().id() || preprocessedData.properties[i].domain().id() === x.range().id()) {
            if (!preprocessedData.properties.includes(x)) {
              preprocessedData.properties.push(x)
            }
          }
        };
      });
    ...
    preprocessedData.nodes = preprocessedData.nodes.filter(function (x) {
      array = []
      for (j = 0; j < preprocessedData.properties.length; j++) {
        if (preprocessedData.properties[j].domain().id() == x.id() || preprocessedData.properties[j].range().id() === x.id()) {
          array.push(x)
          nodesIds.push(x.id())
        }
      };
    ...
    unfilteredData.properties.forEach(function (d) {
      if (nodesIds.includes(d.range().id()) && nodesIds.includes(d.domain().id())) {
        preprocessedData.properties.push(d)
      }
    })

    options.filterModules().forEach(function (module) {
      if (module.setMaxDegreeSetter == undefined) {
        preprocessedData = filterFunction(module, preprocessedData);
      }
    });

    options.compactNotationModule().enabled(true);
    classNodes = preprocessedData.nodes;
    properties = preprocessedData.properties;
    links = linkCreator.createLinks(properties);
    labelNodes = links.map(function (link) {
    ...
    refreshGraphStyle();
    updateHaloStyles();
    graph.options().sidebar().updateSelectionInformation(mapping);
    // if (mapping) {
    //     mapping.highLightMapping()
    // }
    preprocessedData.properties.forEach(function (d) {
      if (d.labelForCurrentLanguage() === "mapping") {
        d.highLightMapping()
      }

    })
    if (mapping) {
      mapping.focused(true);
      mapping.toggleFocus();
  ...
  function setForceLayoutData(classNodes, labelNodes, links, id) {
    var d3Links = [];
        ...
        if (node) {
          if (node.property) {
            ...
            if (node.property().inverse) {
              ...
              if (node.property().labelForCurrentLanguage() == "mapping") {
                var name = node.property().range().labelForCurrentLanguage().toLowerCase() + "=" + node.property().domain().labelForCurrentLanguage().toLowerCase();
              } else {
  ...
  graph.changePropertyStatus = function (element, status) {
    var aProp = element
    aProp.annotations().status[0].value = status
  ...
  graph.mapMode = function (val) {

    var modeOfOpString = d3.select("#modeOfOperationString").node();
    var containerAlignment = d3.select("#containerForAlignment");
    graph.options().setMapModeForDefaultObject(val)
    ...
    var compactNotationContainer = d3.select("#compactnotationModuleCheckbox");
    if (compactNotationContainer) {
      compactNotationContainer.classed("disabled", !mappingMode);
      if (!mappingMode) {
        compactNotationContainer.node().title = "";
        compactNotationContainer.node().disabled = false;
        compactNotationContainer.style("pointer-events", "auto");
    ...
    if (mappingMode === true) {
      containerAlignment.classed("hidden", false);
      d3.select("#zoomSlider").classed("hidden", true);
      graph.editorMode(true);
      options.leftSidebar().showSidebar(0);
      options.leftSidebar().hideCollapseButton(true);
      d3.select("#generalDetails").classed("hidden", false);
      d3.select("#generalDetailsEdit").classed("hidden", true);
      d3.select("#nodeDegreeDistanceSlider").node().disabled = true;

    } else {
      containerAlignment.classed("hidden", true);
      d3.select("#zoomSlider").classed("hidden", false);
      d3.select("#c_node").classed("hidden", true);
      d3.select("#returnAlignmentPane").classed("hidden", true);
      d3.select("#nodeDegreeDistanceSlider").node().disabled = false;
      graph.reset();
  ...
  graph.sanityCheckProperty = function (domain, range, typeString) {
    ...
    if (typeString === "owl:disjointWith" && graph.options().disjointFilter().enabled() === true) {
      graph.options().warningModule().showWarning("Warning",
  ...
  function createNewMappingViaEditor(domain, range, draggerEndposition) {
    // check type of the property that we want to create;                        var range;

    var src = [];
    var tgt = [];
    mappings = {}

    for (i in unfilteredData.nodes) {
      if (unfilteredData.nodes[i].attributes().includes("external")) {
        tgt.push(unfilteredData.nodes[i])
      } else {
        src.push(unfilteredData.nodes[i])
      }
    }
    if (src.includes(domain) && src.includes(range)) {
      return false
    }
    if (tgt.includes(domain) && tgt.includes(range)) {
      return false
    }
    ...
    aProp.range(range);
    aProp.baseIri(d3.select("#iriEditor").node().value);
    aProp.iri(aProp.baseIri() + "#mapping");

    var aPropInv = new propPrototype(graph);
    aPropInv.id("mapping" + eP++);
    aPropInv.domain(range);
    aPropInv.range(domain);
    aPropInv.label("mapping");
    aPropInv.baseIri(d3.select("#iriEditor").node().value);
    aPropInv.iri(aPropInv.baseIri() + "#mapping");
    ...
    aProp.annotations(annot)
    aPropInv.annotations(annot)
    aProp.label("mapping");

    aProp.inverse(aPropInv)
    aPropInv.inverse(aProp)
    ...
    allProps = unfilteredData.properties
    for (j = 0; j < allProps.length; j++) {
      if (allProps[j] === aProp) continue;
      if (allProps[j].domain() === domain && allProps[j].range() === range && allProps[j].type() === aProp.type()) {
        graph.options().warningModule().showWarning("Warning",
          "This mapping already exist!",
          "Element not created!", 1, false);

        newMapping = undefined
        return false;
      }
      if (allProps[j].domain() === range && allProps[j].range() === domain && allProps[j].type() === aProp.type()) {
        graph.options().warningModule().showWarning("Warning",
          "This mapping already exist! ",
          "Element not created!", 1, false);
    ...
    domain.addProperty(aProp);
    range.addProperty(aProp);
    domain.addProperty(aPropInv);
    range.addProperty(aPropInv);
    ...
    unfilteredData.properties.push(aPropInv);
    if (properties.indexOf(aPropInv) === -1)
      properties.push(aPropInv);
    ...
    aProp.labelObject().y = pY;
    aProp.labelObject().py = pY;
    aPropInv.labelObject().x = pX;
    aPropInv.labelObject().px = pX;
    aPropInv.labelObject().y = pY;
    aPropInv.labelObject().py = pY;

    aProp.frozen(graph.paused());
    aProp.locked(graph.paused());
    aPropInv.frozen(graph.paused());
    aPropInv.locked(graph.paused());
    domain.frozen(graph.paused());
    domain.locked(graph.paused());
    range.frozen(graph.paused());
    range.locked(graph.paused());
    ...
    options.focuserModule().handle(aProp);
    options.focuserModule().handle(aPropInv);
    graph.activateHoverElementsForProperties(true, aProp, false, touchDevice);
    graph.activateHoverElementsForProperties(true, aPropInv, false, touchDevice);
    aProp.labelObject().increasedLoopAngle = true;
    aProp.enableEditing(autoEditElement);
  ...
  graph.createNewMapping = function (domainLabel, rangeLabel) {
    var range;
    var domain;
    var src = [];
    var tgt = [];
    ...
    for (i in unfilteredData.nodes) {
      if (unfilteredData.nodes[i].labelForCurrentLanguage()) {
        stringList.push(unfilteredData.nodes[i].labelForCurrentLanguage().toLowerCase())
      }
      if (unfilteredData.nodes[i].attributes().includes("external")) {
        tgt.push(unfilteredData.nodes[i])
      } else {
    ...
    for (i in src) {
      if (src[i].labelForCurrentLanguage()) {
        if (src[i].labelForCurrentLanguage().toLowerCase() === rangeLabel.toLowerCase()) {
          var range = src[i]
          break
        } else {
          var range = undefined
    ...
    for (i in tgt) {
      if (tgt[i].labelForCurrentLanguage()) {
        if (tgt[i].labelForCurrentLanguage().toLowerCase() === domainLabel.toLowerCase()) {
          var domain = tgt[i]
          break
        } else {
    ...
    if (!stringList.includes(rangeLabel.toLowerCase())) {
      graph.options().warningModule().showWarning("Warning",
        "Source name invalid",
        "Element not created!", 1, false);
    ...
    if (!stringList.includes(domainLabel.toLowerCase())) {
      graph.options().warningModule().showWarning("Warning",
        "Target name invalid",
        "Element not created!", 1, false);
      d3.select("#containerForAlignment").classed("hidden", false);
    ...
    if (domain === undefined || range === undefined) {
      graph.options().warningModule().showWarning("Warning",
        "You can't add mapping between classes of the same ontology",
        "Element not created!", 1, false);
      d3.select("#containerForAlignment").classed("hidden", false);
      return false
    }
    ...
    for (i = 0; i < allProps.length; i++) {
      if (allProps[i] === aProp) continue;
      if (allProps[i].domain() === domain && allProps[i].range() === range && allProps[i].type() === aProp.type()) {
        graph.options().warningModule().showWarning("Warning",
          "This mapping already exist!",
          "Element not created!", 1, false);
        graph.mapMode(true);
      ...
      if (allProps[i].domain() === range && allProps[i].range() === domain && allProps[i].type() === aProp.type()) {
        graph.options().warningModule().showWarning("Warning",
          "This mapping already exist! ",
          "Element not created!", 1, false);
        graph.mapMode(true);
  ...
  graph.removeWarning = function (property) {
    graph.options().warningModule().responseWarning(
      "Removing elements",
      "You are about to delete a mapping from the list",
      "Awaiting response!", graph.removePropertyViaEditor, [property, property.inverse()], false);
    graph.fastUpdate()
    graph.options().alignmentPane().setup();
  ...
  graph.removePropertyViaEditor = function (property) {
    ...
    graph.getUpdateDictionary();
    options.focuserModule().handle(undefined);
    if (property.id() !== currentId && property.inverse().id() !== currentId && allIds.includes(currentId)) {

      graph.subgraphGenerator(currentId, currentDegree);
    }
    graph.options().sidebar().updateSelectionInformation();

    if (property === currentMapping) {
  ...
  graph.activateHoverElementsForProperties = function (val, property, inversed, touchBehaviour) {
    ...
    if (val === true) {
      ...
      if (graph.options().drawPropertyDraggerOnHover() === true) {
        if (mappingMode === true && property.labelForCurrentLanguage() !== "mapping") {
