module.exports = function () {
  ...
  options.disjointFilter = function (val) {
    if (!arguments.length) return disjointFilter;
    disjointFilter = val;
  };
  ...
  options.mappingFilter = function (val) {
    if (!arguments.length) return mappingFilter;
    mappingFilter = val;
  };
  ...
  options.alignmentPane = function (val) {
    if (!arguments.length) return alignmentPane;
    alignmentPane = val;
  };
  ...
  defaultOptionsConfig.mapMode = "true";
  ...
  defaultOptionsConfig.filter_mapping = "false";
  ...
  options.setMapModeForDefaultObject = function (val) {
    defaultOptionsConfig.mapMode = String(val);
  };
  ...
  function updateConfigObject() {
    ...
    defaultOptionsConfig.filter_mapping = String(options.filterMenu().getCheckBoxValue("mappingFilterCheckbox"));
  ...
  options.tgtSearchMenu = function (m) {
    if (!arguments.length) return tgtSearchMenu;
    tgtSearchMenu = m;
    return options;
  };

  options.srcSearchMenu = function (m) {
    if (!arguments.length) return srcSearchMenu;
    srcSearchMenu = m;
    return options;
  };
  ...
  options.setOptionsFromURL = function (opts, changeEditFlag) {
    ...
    var settingFlag = false;
    if (opts.mapMode) {
      if (opts.mapMode === "true") settingFlag = true;
      d3.select("#mapModeModuleCheckbox").node().checked = settingFlag;

      if (changeEditFlag && changeEditFlag === true) {
        graphObject.mapMode(settingFlag);
      }

      // update config object
      defaultOptionsConfig.mapMode = opts.mapMode;
    ...
    settingFlag = false;
    if (opts.filter_mapping) {
      if (opts.filter_mapping === "true") settingFlag = true;
      filterMenu.setCheckBoxValue("mappingFilterCheckbox", settingFlag);
      defaultOptionsConfig.filter_mapping = opts.filter_mapping;
    }
