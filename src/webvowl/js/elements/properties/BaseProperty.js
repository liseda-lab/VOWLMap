...
module.exports = (function (){
               ...
               var Base = function(graph) {
                   ...
                   this.toggleFocus = function() {
                       if (that.inverse()) {
                           if (that.inverse().id() == graph.getCurrentId()) {
                               labelElement.select("rect").classed("focused", true);
                               that.inverse().labelElement().select("rect").classed("focused", true);
                           } else {
                   ...
                   this.highLightMapping = function() {
                       if (that.annotations()) {

                           if (that.annotations().status) {
                               if (that.annotations().status[0].value === "unreviewed") {
                                   labelElement.select("rect").classed("unreviewed", true);
                               } else if (that.annotations().status[0].value === "correct") {
                                   labelElement.select("rect").classed("correct", true);
                               } else if (that.annotations().status[0].value === "incorrect") {
                                   labelElement.select("rect").classed("incorrect", true);
                               } else if (that.annotations().status[0].value === "unsure") {
                                   labelElement.select("rect").classed("unsure", true);
                               }
                           }
                       }
                   ...
                   this.draw = function(labelGroup) {
                       ...
                       //For mappings, it is only shown 1 label for both property and inverse.
                       if (that.inverse()) {

                           var yTransformation = (that.height() / 2) + 1 /* additional space */ ;
                           that.inverse()
                               .labelElement(attachLabel(that.inverse()));
                           if (that.inverse().labelForCurrentLanguage() !== "mapping") {
                               that.labelElement()
                   ...
                   this.drawLabel = function(labelContainer) {
                       ...
                       if (this.labelForCurrentLanguage() == "mapping") {
                           if (this.annotations().score) {
                               textElement.addText(this.annotations().score[0].value, "", suffixForFollowingEquivalents);
                           }
                       } else {
                           textElement.addText(this.labelForCurrentLanguage(), "", suffixForFollowingEquivalents);
                       }
